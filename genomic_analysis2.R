...
# 1. ADMIXTURE bar plot (K=10, optimal)
q_mat <- as.matrix(read.table("rice_pruned2.10.Q"))
fam <- read.table("rice_pruned2.fam")
samples <- fam[, 2]
...
pdf("admixture_K10_barplot_v2.pdf", width=12, height=4)
par(mar=c(4,4,1,1))
barplot(t(q_mat_ord), col=rainbow(10), border=NA,
        xlab="Individuals", ylab="Ancestry",
        main="ADMIXTURE Ancestry Proportions (K=10, 409 samples)")
dev.off()
cat("ADMIXTURE bar plot saved\n")

# CV error plot
cv_errors <- c(1.03661, 0.79855, 0.69085, 0.66519,
               0.60875, 0.58742, 0.58982, 0.56551,
               0.56252, 0.54903)
pdf("admixture_cv_errors_v2.pdf", width=6, height=4)
plot(1:10, cv_errors, type="b", pch=19, col="blue",
     xlab="K", ylab="CV error",
     main="ADMIXTURE Cross-Validation Error (filtered)")
dev.off()
cat("CV error plot saved\n")

# 2. DAPC
ped_file <- "/mnt/d/Rice accessions/RiceDiversity.44K.MSU6.Genotypes_PLINK/RiceDiversity_44K_Genotypes_PLINK/rice_pruned2.raw"
...
if (file.exists(kinship_file)) {
  ...
  pdf("kinship_heatmap_v2.pdf", width=10, height=9)
  ...
  pdf("kinship_distribution_v2.pdf", width=6, height=4)
