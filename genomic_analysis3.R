...
# 1. ADMIXTURE bar plot (K=10)
q_mat <- as.matrix(read.table("rice_pruned3.10.Q"))
...
pdf("admixture_K10_barplot_v3.pdf", width=12, height=4)
par(mar=c(4,4,1,1))
barplot(t(q_mat_ord), col=rainbow(10), border=NA,
        xlab="Individuals", ylab="Ancestry",
        main="ADMIXTURE Ancestry Proportions (K=10, 379 samples)")
dev.off()
cat("ADMIXTURE bar plot saved\n")

# CV error plot
cv_errors <- c(1.04874, 0.81173, 0.69596, 0.64423,
               0.61773, 0.61044, 0.59585, 0.57622,
               0.56504, 0.55556)
pdf("admixture_cv_errors_v3.pdf", width=6, height=4)
plot(1:10, cv_errors, type="b", pch=19, col="blue",
     xlab="K", ylab="CV error",
     main="ADMIXTURE Cross-Validation Error (QC3b, 379 samples)")
dev.off()
...
if (file.exists(kinship_file)) {
  ...
  pdf("kinship_heatmap_v3.pdf", width=10, height=9)
  heatmap(kins, col=colorRampPalette(c("white","yellow","red"))(100),
          main="Kinship Matrix Heatmap (full set)", cexRow=0.5, cexCol=0.5)
  dev.off()
  cat("Kinship heatmap saved\n")

  pdf("kinship_distribution_v3.pdf", width=6, height=4)
